...
function getLetfInfo(ticker: string): { underlying: string; leverage: number } | null {
  return LETF_LOOKUP[(ticker || "").toUpperCase().trim()] || null;
}

function buildEntryEmbed(data: Record<string, any>, ticker: string): DiscordPreviewMessage {
  ...
  const displayTicker = isLETF && letfInfo ? `${letfInfo.underlying} \u2192 ${ticker}` : ticker;
  ...
  if (instrumentType === "Options") {
    ...
    fields.push(
      { name: "\ud83d\udfe2 Ticker", value: letfInfo?.underlying || ticker, inline: true },
      { name: "\ud83d\udcca Stock Price", value: stockPrice ? fmtPrice(stockPrice) : "\u2014", inline: true },
      { name: "\ud83d\udcc8 Direction", value: direction, inline: true },
      { ...SPACER },
      { name: "\ud83d\udcf9 LETF", value: `${ticker} (${letfInfo?.leverage || "?"}x ${dir})`, inline: true },
      { name: "\ud83d\udcb0 LETF Entry", value: entryPrice ? fmtPrice(entryPrice) : "Pending", inline: true },
      { name: "\ud83d\uded1 Stop", value: data.stop_loss != null ? `${fmtPrice(Number(data.stop_loss))} (${fmtPct(entryForPct, Number(data.stop_loss))})` : "\u2014", inline: true },
      { ...SPACER },
    );
    ...
    fields.push(
      { name: "\ud83d\udfe2 Ticker", value: ticker, inline: true },
      { name: "\ud83d\udcca Stock Price", value: entryPrice ? fmtPrice(entryPrice) : "\u2014", inline: true },
      { name: "\ud83d\udcc8 Direction", value: direction, inline: true },
      { ...SPACER },
    );
...
function addTradePlan(fields: DiscordField[], data: Record<string, any>, refPrice: number | null): void {
  const parts: string[] = [];
  if (data.targets && typeof data.targets === "object") {
    ...
    if (prices.length > 0) parts.push(`\ud83c\udfaf Targets: ${prices.join(", ")}`);
  }
  ...
  if (parts.length > 0) {
    fields.push({ name: "\ud83d\udcdd Trade Plan", value: parts.join("\n"), inline: false });
  }
}

function addTakeProfitPlan(fields: DiscordField[], data: Record<string, any>, refPrice: number | null): void {
...
function buildTargetHitEmbeds(data: Record<string, any>, ticker: string): DiscordPreviewMessage[] {
  ...
  return entries.map(([key, val], idx) => {
    ...
    const pctProfit = entryPrice && entryPrice > 0
    ...
    const fields: DiscordField[] = [
      { name: "\u2705 Entry", value: fmtPrice(entryPrice), inline: true },
      { name: "\ud83c\udfaf TP Hit", value: fmtPrice(targetPrice), inline: true },
      { name: "\ud83d\udcb8 Profit", value: pctProfit ? `+${pctProfit}%` : "\u2014", inline: true },
      { ...SPACER },
      { name: `\ud83d\udea8 Status: TP ${key.toUpperCase()} Reached \ud83d\udea8`, value: "\u200b", inline: false },
    ];
...
function buildStopLossHitEmbed(data: Record<string, any>, ticker: string): DiscordPreviewMessage | null {
  ...
  const pctLoss = entryPrice && entryPrice > 0
  ...
  const fields: DiscordField[] = [
    ...
    { name: "\ud83d\udcb8 Result", value: pctLoss ? `${pctLoss}%` : "\u2014", inline: true },
    { ...SPACER },
    { name: "\ud83d\udea8 Status: Stop Loss Hit \ud83d\udea8", value: "\u200b", inline: false },
  ];
...
function buildTradeClosedEmbed(data: Record<string, any>, ticker: string): DiscordPreviewMessage {
  const instrumentType = data.instrument_type || "Shares";
  const isLETF = instrumentType === "LETF";
  const letfInfo = isLETF ? getLetfInfo(ticker) : null;
  const entryPrice = data.entry_price ? Number(data.entry_price) : null;
